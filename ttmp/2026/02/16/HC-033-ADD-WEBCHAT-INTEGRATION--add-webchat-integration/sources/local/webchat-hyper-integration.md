...
RelatedFiles: []
ExternalSources: []
Summary: Imported external design/analysis document provided by user.
LastUpdated: 2026-02-16T12:12:00-05:00
WhatFor: Preserve the imported source material used for validation.
WhenToUse: Read for original architecture narrative and proposed phased implementation details.
---
...
The bootstrap stores card definitions in a VM-local registry and exposes__stackHost methods like:
...
renderCard(sessionId, cardId, ...): calls__stackHost.render(...) and validates the resulting UI tree with zod (uiSchema).
...
# Step-by-step implementation plan (Pinocchio + Geppetto backend, HyperCard frontend)

This plan assumes we’re building:

- **A Go backend** using **Pinocchio `pkg/webchat`** (transport, streaming, timeline) + **Geppetto** (LLM engine, middleware, tools, structured sinks).
- **A HyperCard Inventory frontend** (the React/Redux “desktop” + QuickJS plugin card runtime) that:

  - streams SEM events from the backend over WebSocket,
  - renders **inline widgets** in chat,
  - allows “**Create card**” to materialize chat results into cards,
  - later: can accept **LLM-proposed plugin card code** and inject it into the VM.

We’ll follow the same “handler-first” pattern as **`pinocchio/cmd/web-chat`**:

- app-owned routes: `POST /chat`, `GET /ws`, `GET /api/timeline`
- backend emits **SEM frames** over websocket
- frontend optionally hydrates using timeline

---

## 0) One-time decisions (do this before coding)

### 0.1 Define the “artifact” approach for cards (MVP)

To keep the first iteration simple and reliable:

- The LLM produces:

  - normal assistant text (streamed)
  - optional structured blocks for **widgets/artifacts** (reports, item summaries, tables)
- The frontend:

  - renders those artifacts inline in chat
  - when user clicks **Create card**, it opens a **template plugin card** (e.g., `reportViewer`) with `param=<artifactId>`

This avoids dynamic card-definition injection on day 1, but still gives “create new cards” behavior (new “instances” using `param`).

### 0.2 Defer dynamic plugin card injection to Phase 6

We’ll still design for it early (structured sinks + event types), but ship it after:

- streaming chat works,
- inventory tools work,
- artifact→card materialization works.

---

## 1) Repo setup + working baseline build

### 1.1 Backend prerequisites

- Go toolchain working for the Pinocchio repo
- (Optional, later) Redis for stream backend scaling

### 1.2 Frontend prerequisites

In `2026-02-12--hypercard-react/`:

- `npm install`
- `npm run dev -w apps/inventory` should run

**Definition of done**

- Both repos build locally.

---

## 2) Backend MVP: create `cmd/hypercard-inventory-server`

Create a new command modeled on `pinocchio/cmd/web-chat` but WITHOUT its embedded frontend.

### 2.1 Create the new command folder

Create:

- `pinocchio/cmd/hypercard-inventory-server/main.go`
- `pinocchio/cmd/hypercard-inventory-server/runtime_composer.go`
- `pinocchio/cmd/hypercard-inventory-server/request_resolver.go`

### 2.2 Implement `main.go` (copy the pattern from `cmd/web-chat/main.go`)

**Hard requirements (copy these concepts exactly):**

- Build a `webchat.Server` via `webchat.NewServer(...)`
- Register middleware factories on the server (`srv.RegisterMiddleware`)
- Register tool factories on the server (`srv.RegisterTool`)
- Mount app-owned handlers:

  - `webhttp.NewChatHandler(srv.ChatService(), resolver)`
  - `webhttp.NewWSHandler(srv.StreamHub(), resolver, websocket.Upgrader{...})`
  - `webhttp.NewTimelineHandler(srv.TimelineService(), logger)`
- Assign mux to `srv.HTTPServer().Handler`
- Run `srv.Run(ctx)`

**Minimal mux (do exactly this):**

- `POST /chat` and `POST /chat/` → chat handler
- `GET /ws` → ws handler
- `GET /api/timeline` → timeline handler
- `GET /api/*` → `srv.APIHandler()` (optional but recommended for debug and parity)

### 2.3 Implement a strict request resolver

In `request_resolver.go`, implement `ConversationRequestResolver` like the web-chat resolver but simpler:

- For `GET /ws`:

  - require `conv_id` query param
  - set `RuntimeKey = "inventory"`
  - `Overrides = nil` (MVP)
- For `POST /chat`:
  ...
  - parse JSON body:
    ...
    - `prompt` (or `text`)
    - optional `conv_id` (generate if empty)
  - set `RuntimeKey = "inventory"`
  - ignore client overrides initially (security + consistency)
...
- `curl` can:
  ...
  - `POST /chat` with `{"conv_id":"...","prompt":"hello"}` and receive streaming `llm.*` SEM frames.

---

## 3) Backend: Inventory “domain” storage + tools (no widgets yet)

This phase gives the LLM something real to do.

### 3.1 Add a SQLite inventory DB

Create a backend package:

- `pinocchio/pkg/hypercard_inventory/store/`
...
- tables:
  ...
  - `items(sku TEXT PRIMARY KEY, name TEXT, category TEXT, tags TEXT, qty INTEGER, price REAL, cost REAL)`
  - `sales(id TEXT PRIMARY KEY, date TEXT, sku TEXT, qty INTEGER, total REAL)`

Also add a one-time seed function that loads the same seed data as the frontend `apps/inventory/src/domain/seedData.ts`.

**Definition of done**

- backend starts with a non-empty inventory table.

### 3.2 Implement Geppetto tools (Go functions)

Create:

- `pinocchio/cmd/hypercard-inventory-server/tools_inventory.go`

Follow the same tool pattern used by `pinocchio/cmd/agents/simple-chat-agent/pkg/tools/tools.go`:

- use `tools.NewToolFromFunc(...)`

Implement these tools (names matter; keep them stable):

- `inventory_search_items(query: string) -> { items: [...] }`
- `inventory_get_item(sku: string) -> { item: ... }`
- `inventory_low_stock(threshold: number) -> { items: [...] }`
- `inventory_report(threshold: number, since: string) -> { summary: {...}, lowStock:[...], outOfStock:[...], recentSales:[...] }`
- `inventory_update_qty(sku: string, delta: number) -> { item: ... }`
- `inventory_record_sale(sku: string, qty: number, date?: string) -> { sale: ... }`

Register them in `main.go` with:

- `srv.RegisterTool("inventory_search_items", ...)` etc.

### 3.3 Runtime composer: default system prompt + allowed tools

Implement `runtime_composer.go` like `cmd/web-chat/runtime_composer.go`, but:

- `runtimeKey` fixed to `"inventory"`
- system prompt is **inventory-specific**
- `AllowedTools` includes the inventory tools (and optionally calculator)

**System prompt must explicitly say:**

- “Use tools for inventory facts; don’t invent SKUs or quantities”
- “When you generate a report, prefer calling `inventory_report`”

**Definition of done**

- Asking “What’s low stock under 3?” results in tool calls, not hallucination.

---

## 4) Backend: Structured widget extraction using Geppetto structured sinks

Now we add the “inline report widget in chat” backbone.

### 4.1 Pick the structured tag format (freeze this)

Use Geppetto structured sink tag format:

- `<hypercard:Widget:v1> ... </hypercard:Widget:v1>`
...
title: "..."
artifact:
  # arbitrary JSON-like YAML the frontend will store
ui:
  # UINode-like structure or a simpler schema
actions:
  - label: "Create card"
    action:
      kind: "create_card"
...
Important: The **structured sink will strip these blocks from the visible assistant text**, but emit typed events.

### 4.2 Implement extractor + event types

Create package:

- `pinocchio/pkg/hypercard_sem_widgets/`
...
Define a Geppetto event type string, e.g.:

- `hypercard.widget.v1`

Implement:

- `type EventHypercardWidgetV1 struct { events.EventImpl; Kind string; Title string; Artifact map[string]any; UI map[string]any; Actions []... }`

In `init()`:

- register event factory via `geppetto/events.RegisterEventFactory("hypercard.widget.v1", func() events.Event { return &EventHypercardWidgetV1{} })`

#### Extractor

Implement a `structuredsink.Extractor`:

- `TagPackage() -> "hypercard"`
- `TagType() -> "Widget"`
- `TagVersion() -> "v1"`
- `OnCompleted(...)` parses YAML and returns `EventHypercardWidgetV1`

### 4.3 Wrap the sink using `WithEventSinkWrapper`

In your server creation (`webchat.NewServer`), pass:

- `webchat.WithEventSinkWrapper(...)`

Wrapper implementation:

- take downstream sink
- return `structuredsink.NewFilteringSink(downstream, opts, widgetExtractor /*, later more extractors */)`

Use options:

- `Malformed: MalformedErrorEvents` (MVP)
- `MaxCaptureBytes`: set something sane (e.g. 256KB)

### 4.4 Ensure widgets reach the websocket: add SEM mapping

Right now: the websocket stream only forwards **SEM frames**.
So you must add a SEM mapping handler for your custom event type.

Create:

- `pinocchio/pkg/hypercard_sem_widgets/sem_mapping.go`

In `init()`:

- `semregistry.RegisterByType[*EventHypercardWidgetV1](func(ev *EventHypercardWidgetV1) ([][]byte, error) { ... })`

Emit a SEM frame like:

- `type: "hypercard.widget.v1"`
- `id: "<uuid>"` (use `ev.Metadata().ID` or generate)
- `data: { kind, title, artifact, ui, actions }`

**Definition of done**

- When the LLM outputs a `<hypercard:Widget:v1>` block, the frontend receives a websocket SEM event of type `hypercard.widget.v1`.

---

## 5) Backend: Make the LLM actually emit widget blocks (middleware/system prompt)

### 5.1 Add “Widget emission rules” to the system prompt

Update the inventory system prompt to include:

- When user asks for:

  - report → emit a Widget block with `kind: report`
  - item lookup → emit `kind: item`
- Always include:

  - `title`
  - `artifact` (raw data the app can store)
  - `actions` (at minimum: “Create card” with correct template)

Also include 1–2 examples in the prompt (short, realistic).

### 5.2 Keep the visible assistant text clean

Tell the model:

- Put the structured block at the end.
- Don’t reference the tag in user-visible prose.
  FilteringSink will remove it anyway, but this reduces risk.

**Definition of done**

- Asking “Give me a low stock report under 3” yields:

  - streamed assistant narrative
  - plus a widget event containing report data + action “Create card”.

---

## 6) Frontend MVP: connect HyperCard Inventory app to Pinocchio backend

We’ll build a real chat surface inside HyperCard and show inline widgets.

### 6.1 Add Vite proxy for backend routes (dev ergonomics)

Edit `apps/inventory/vite.config.ts`:
...
### 6.2 Build a Pinocchio chat transport client (frontend)

Create:

- `apps/inventory/src/features/chat/pinocchioTransport.ts`

Responsibilities:

- `getOrCreateConvId()` (store in localStorage)
- `connectWS(convId, onFrame)`

  - open `/ws?conv_id=...`
  - parse incoming JSON
  - dispatch Redux actions with parsed SEM events
- `sendPrompt(convId, text)`

  - POST `/chat` with `{ conv_id, prompt }`

Follow Pinocchio’s expected frontend sequence:

1. conv_id
2. open ws
3. (optional) fetch `/api/timeline`
4. send prompt

For MVP you may skip hydration and just stream.

### 6.3 Update the inventory app chat slice to consume SEM events

You already have `apps/inventory/src/features/chat/chatSlice.ts`.

Refactor it to store:
...
- `messages[]` with:
  ...
  - `text`
  - `content[]` blocks (for widgets)
  - `actions[]` (for “Create card”)

Implement reducers for SEM events:

- `chat.message` → append user message
- `llm.start` → append new assistant message with status streaming
- `llm.delta` → append text to last streaming assistant message
- `llm.final` → finalize the last streaming assistant message
- `hypercard.widget.v1` → attach widget content + actions to the **current assistant message**

**Rule (MVP):** attach widgets to “most recent assistant message still streaming or most recent assistant message.”

### 6.4 Render a chat window as an app-window inside DesktopShell

In `apps/inventory/src/App.tsx`:

- pass `renderAppWindow` to `DesktopShell`
- on initial mount, dispatch `openWindow(...)` for an app window:

  - `content.kind = "app"`
  - `appKey = "inventoryChat"`

Create component:

- `apps/inventory/src/features/chat/InventoryChatWindow.tsx`

Use engine’s existing UI:

- `@hypercard/engine` `ChatWindow` component (it supports inline widgets already)

Wire:

- `messages` from chat slice
- `onSend` calls `sendPrompt(...)`
- `renderWidget` renders:

  - for widget.type `"uinode"`: reuse `PluginCardRenderer` with `tree=...`
  - OR render a simple `<table>` for MVP

**Definition of done**

- Inventory app launches with a chat window.
- Sending a message streams assistant response into that chat window.

---

## 7) Frontend: “Create card” from inline widget (artifact → template card)

Now we connect chat results to HyperCard cards.

### 7.1 Add an “artifacts” domain slice

Create:

- `apps/inventory/src/features/artifacts/artifactsSlice.ts`

State:

- `byId: Record<string, { kind, title, data, ui? }>`
- actions:

  - `artifactUpsert({id, kind, title, data, ui})`

When you receive `hypercard.widget.v1`:

- generate `artifactId` if the event doesn’t provide one
- store it in artifacts slice

### 7.2 Define template cards in the plugin bundle

Update `apps/inventory/src/domain/pluginBundle.vm.js` to include at minimum:

- `reportViewer` card:

  - reads `globalState.domains.artifacts.byId[nav.param]`
  - renders title + table from artifact data
- `itemViewer` card:

  - reads artifact by param
  - renders item summary

Make sure these cards exist in `apps/inventory/src/domain/stack.ts` `cards` list so DesktopShell can open them.

### 7.3 Implement the “Create card” action handling in chat UI

When widget event includes an action like:

```json
{ "kind": "create_card", "template": "reportViewer" }
```

Your chat UI should:

1. ensure artifact is stored (has an id)
2. dispatch `openWindow(...)` for a card window:

   - `content.kind = "card"`
   - `content.card.cardId = template`
   - `content.card.param = artifactId`
   - `cardSessionId = new uuid`

**Definition of done**

- User asks for report
- report widget appears inline
- clicking “Create card” opens a report card window showing the same report.

---

## 8) Upgrade: timeline hydration (so refresh doesn’t lose chat)

### 8.1 Run backend with timeline persistence enabled

Start backend with:

- `--timeline-db ./data/timeline.db`

(Exact flag names match Pinocchio router settings; keep them consistent.)

### 8.2 Implement hydration in frontend

On startup (after ws connect):

- GET `/api/timeline?conv_id=...&since_version=0&limit=...`
- convert snapshot entities into chat messages (MVP: only the message entities you care about)
- then apply buffered ws frames

**Definition of done**

- refresh page → chat history returns.

---

## 9) Phase 6 (advanced): LLM-proposed plugin card code injection into the VM

This is where we enable *new card definitions* that the LLM can propose.

### 9.1 Add a new structured block: `<hypercard:PluginCard:v1>`

Spec YAML:

```yaml
card:
  id: "custom_low_stock_dashboard"
  title: "Low Stock Dashboard"
  icon: "📉"
  description: "Shows low stock and reorder suggestions"
plugin:
  code: |
    ({ ui }) => ({
...
### 9.2 Backend: extractor + event type + SEM mapping

Add:

- `EventHypercardPluginCardV1` with type string `hypercard.plugin_card.v1`
- extractor parses YAML and emits event
- SEM mapping emits `type: "hypercard.plugin_card.v1"` with payload including code string

### 9.3 Frontend: proposal UI + accept flow

In chat message renderer:

- show a “Proposed card” section with title/icon and buttons:
...
- **Preview**: create a temporary QuickJS runtime session and call `defineCard` + `renderCard` to ensure it renders.
- **Accept & Create**:

  1. store accepted plugin card definition in Redux (dynamicCards slice)
  2. inject into existing runtime sessions (call `runtimeService.defineCard`)
  3. open a window with that cardId

### 9.4 Make dynamic cards openable (engine changes likely needed)

Right now `PluginCardSessionHost` only allows navigation to cards present in `stack.cards`.

You’ll need to change that logic so it also accepts:

- `dynamicCards.byId` card ids

Minimal approach:

- keep `stack.cards` for built-ins
- add `dynamicCards` to Redux
- in `PluginCardSessionHost`, replace:

  - `stack.cards[currentNav.card] ? currentNav.card : stack.homeCard`
    with:
  - `isKnownCard(currentNav.card) ? currentNav.card : stack.homeCard`
    where `isKnownCard` checks stack.cards OR dynamicCards.

Also update desktop icons / menus later if needed.

**Definition of done**

- LLM proposes a new plugin card
- user accepts
- card is injected and opened successfully.

---

## 10) Delivery plan for junior devs (PR-by-PR checklist)

### PR1 — Backend skeleton

- new `cmd/hypercard-inventory-server`
- /chat + /ws + /api/timeline streaming works (basic “You are an assistant” prompt)

### PR2 — Inventory DB + tools

- sqlite schema + seed
- tools registered
- model uses tools

### PR3 — Widget structured sink

- `<hypercard:Widget:v1>` extractor
- SEM mapping sends `hypercard.widget.v1`

### PR4 — Frontend transport + streaming

- inventory app opens chat window
- streams `llm.*` events

### PR5 — Inline widget render

- widget events render inline
- “Create card” opens template card with param

### PR6 — Timeline hydration

- refresh restores chat

### PR7 — PluginCard proposals (advanced)

- `<hypercard:PluginCard:v1>` end-to-end
- accept injects into VM
- open dynamic card
