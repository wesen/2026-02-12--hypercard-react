...
export function computeReportSections(items: Item[], sales: SaleEntry[], threshold: number): Record<string, string> {
  return {
    ...
    bestMargin: (() => {
      ...
      return sorted[0]
        ? `${sorted[0].sku} ${sorted[0].name} (${(((sorted[0].price - sorted[0].cost) / sorted[0].price) * 100).toFixed(0)}%)`
        : '—';
    })(),
    recentSalesTotal:
      '$' +
      sales
        .filter((s) => s.date >= '2026-02-08')
